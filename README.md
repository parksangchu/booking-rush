...
Redis Counter는 **락 없이** INCR 원자 연산으로 동시성 해결. 거절 대부분이 Redis에서만 처리되어 DB 부하가 크게 줄어든다. 하지만 발급 성공 시 **DB INSERT가 동기로 실행**되어 2,000 RPS에서 악화 시작.
...
자세한 전략 비교: [docs/coupon/strategy-comparison.md](docs/coupon/strategy-comparison.md)
